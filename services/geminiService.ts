...
**작성 가이드라인:**
1.  전체 보고서 초안은 **"주제목 - 부제목 - 내용"**의 구조를 엄격하게 따라야 합니다.
2.  **주제목:** '# 제목' 형식으로, 이미 제공된 '**선택된 제목 (주제목)**'을 그대로 사용합니다. (예: '# 스마트 재난관리로 멈춤 없는 고속도로 구현')
3.  **부제목:** 주제목 아래에, 전체 내용을 함축적으로 보여주는 1~2줄의 설득력 있는 부제목을 생성해주세요. (예: '## IoT, AI 기반 예측적 재난대응 체계 구축으로 국민 안전 확보 및 운행 안정성 제고')
4.  **내용:** 아래 항목들에 대해, **완전한 서술형 문장이 아닌, 핵심 내용을 담은 간결한 개조식(bullet point) 형태**로 요약해주세요.
    -   **추진배경:** (기존의 문제점, 목표 등)
    -   **주요 내용:** (핵심 활동, 혁신 수단 등)
    -   **주요 성과:** (기관 및 고객 체감 성과)
5.  내용은 **"문제점 → 개선 활동 → 정량/정성적 성과"**의 논리 구조가 드러나도록 작성하고, 가능한 한 구체적인 데이터를 포함해주세요.
6.  각 항목의 핵심 내용을 가장 효과적으로 표현할 수 있는 **도표나 그림(예: 막대그래프, 순서도, 표 등)을 구체적으로 제안**해주세요.
7.  도표/그림 제안은 반드시 \`> 📊 **도표/그림 제안**\` 형식으로 시작하며, **유형, 제목, 설명**을 포함해야 합니다.
8.  전체 내용은 마크다운(Markdown) 형식으로 구조화하여 명료하게 전달해주세요.
`;
};
...
2.  기존의 **"주제목 - 부제목 - 내용" 구조와 개조식(bullet point) 형식, 논리 구조("문제점 → 개선 활동 → 성과")를 반드시 유지**해주세요.
