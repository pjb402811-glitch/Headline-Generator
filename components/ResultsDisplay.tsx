import type { Headline, HeadlineResult } from '../types';
  ...
  return (
    <div className="space-y-12">
      <div className="text-center">
         ...
         <p className="mt-2 text-lg text-gray-400">15가지 유형별로 총 {totalHeadlines}개의 제목이 생성되었습니다. 마음에 드는 제목 하나를 선택하여 보고서 초안을 작성해보세요.</p>
      </div>
          ...
          <div key={index} className="bg-gray-800 p-6 rounded-lg shadow-md border border-gray-700">
            ...
            <ul className="space-y-4">
              {result.headlines.map((headline, headlineIndex) => (
                <li key={headlineIndex} className={`p-4 rounded-md border transition-colors duration-200 ${selectedHeadline?.title === headline.title && selectedHeadline?.strategy === headline.strategy ? 'bg-blue-900/50 border-blue-600 ring-2 ring-blue-500' : 'bg-gray-700/50 border-gray-600'}`}>
                  <label className="flex items-start cursor-pointer">
                    <input 
                      type="radio"
                      name="selectedHeadline"
                      className="h-5 w-5 mt-1 mr-4 text-blue-600 bg-gray-700 border-gray-500 focus:ring-blue-500"
                      checked={selectedHeadline?.title === headline.title && selectedHeadline?.strategy === headline.strategy}
                      onChange={() => onSelectHeadline(headline)}
                    />
      ...
      <div className="text-center mt-12 flex flex-col sm:flex-row flex-wrap justify-center items-center gap-4">
        <button
          ...
          disabled={!selectedHeadline || isDrafting || isRegenerating}
          className="w-full sm:w-auto px-8 py-3 bg-indigo-600 text-white font-bold rounded-md hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 transition-transform transform hover:scale-105 disabled:bg-gray-600 disabled:cursor-not-allowed flex items-center justify-center order-first sm:order-none"
        >
          {isDrafting ? (
            <>
        ...
        <button
          ...
          disabled={isRegenerating || isDrafting}
